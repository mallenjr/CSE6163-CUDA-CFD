...
void computeResidual_kernel(float *presid, float *uresid, float *vresid, float *wresid,
  ...
  float area = dy*dz;
  for(int j=0;j<nj;++j) {
    int offset = k + kstart+i*iskip+j*jskip;
  ...
  for(int j=0;j<nj+1;++j) {
    int indx = k+kstart+i*iskip+j*jskip;
...
//		       ni, nj, nk, kstart, iskip, jskip, L);
  ...
  while(simTime < stopTime) {
    ...
    computeResidual_kernel<<<ni, nk>>>(presid_cuda, uresid_cuda, vresid_cuda, wresid_cuda,
